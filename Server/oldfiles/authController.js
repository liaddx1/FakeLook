...
class AuthController {
    addUser = async (req, res) => {
        ...
        try {
            const result = await httpService.post(`${authRoute}/register`, req.body).then((response) => {
    ...
    userLogIn = async (req, res) => {
        ...
        try {
            const result = await axios.post('http://localhost:8081/login', req.body).then((response) => {
                res.status(200).send({ auth: true, userId: response.recordset[0].userId, authToken: token });
            })
            .catch((error) => {console.log(error)})
        }
